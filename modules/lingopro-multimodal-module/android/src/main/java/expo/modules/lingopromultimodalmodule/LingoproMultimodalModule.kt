...
class LingoproMultimodalModule : Module() {
    ...
    fun summarizeTools(tools: List<JSONObject>): String {
        return tools.mapIndexed { index, tool ->
            ...
            val required = if (requiredArray != null && requiredArray.length() > 0) {
                ...
                "bulkInsertWords" -> {
                    ...
                    val wordsToInsert = (0 until wordsArray.length()).map { i ->
                        ...
                        Word(
                            ...
                            language = wordObj.getString("language"),
                            word = wordObj.getString("word"),
                            meaning = wordObj.getString("meaning"),
                            writing = wordObj.optString("writing", wordObj.getString("word")),
                            wordType = wordObj.optString("wordType", "unknown"),
                            category1 = wordObj.getString("category1"),
                            // Correctly handles nullability by converting an empty string to null
                            category2 = wordObj.optString("category2").takeIf { it.isNotBlank() },
                            phonetics = wordObj.optString("phonetics").takeIf { it.isNotBlank() },
                            tags = wordObj.optJSONArray("tags")?.let { tagsArray ->
    ...
    override fun definition() = ModuleDefinition {
        ...
        AsyncFunction("bulkInsertWords") { words: List<Map<String, Any?>>, promise: Promise ->
            try {
                val wordObjects = words.map { wordMap ->
                    val tags = (wordMap["tags"] as? List<*>)?.filterIsInstance<String>() ?: emptyList()
                    Word(
                        id = 0, // ID is auto-generated by the database
                        language = wordMap["language"] as String,
                        word = wordMap["word"] as String,
                        meaning = wordMap["meaning"] as String,
                        writing = wordMap["writing"] as? String ?: wordMap["word"] as String,
                        wordType = wordMap["wordType"] as? String ?: "unknown",
                        category1 = wordMap["category1"] as String,
                        // The values are already nullable from the map
                        category2 = wordMap["category2"] as? String,
                        phonetics = wordMap["phonetics"] as? String,
                        // Unchecked cast is a warning, as the compiler can't guarantee the List<Any> is
                        // a List<String>. This is the standard way to handle dynamic types from JS.
                        tags = tags
                    )
                }
        ...
        AsyncFunction("getWordById") { id: Long?, promise: Promise ->
            try {
                when {
                    id == null -> {
                ...
                promise.reject(
                    "DB_QUERY_ERROR",
                    "Failed to get word with ID $id: ${e.message}",
                    e
                )
            }
        }
        ...
        AsyncFunction("generateTopicCards") {
                handle: Int,
                requestId: Int,
                topic: String,
                language: String,
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    val model = modelMap[handle] ?: run {
                        promise.reject("INVALID_HANDLE", "Model not loaded", null)
                        return@launch
                    }
    ...
    """.trimIndent()
                    ...
                    promise.reject("PARSE_ERROR", "Invalid JSON format: ${e.message}", e)
                } catch (e: Exception) {
        ...
        AsyncFunction("updateSummaries") { rawResponse: String, promise: Promise ->
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    promise.reject("UPDATE_SUMMARIES_ERROR", e.message, e)
                }
            }
        }
        ...
        AsyncFunction("generateResponseAsync") { handle: Int, requestId: Int, prompt: String, imagePath: String, useTools: Boolean, promise: Promise ->
            ...
            """.trimIndent()
                    ...
                    val historyTokens = estimateTokens(modelHistoryContext)
                    if (promptTokens + historyTokens > MAX_TOKENS) {
                        throw Exception("Input exceeds 32K tokens! (Current: $promptTokens + $historyTokens). Please create a new session")
                    }

                    var imagePathSelected = if (imageHistoryPath != NO_IMAGE && imagePath == imageHistoryPath) {
                    ...
                    var toolsJsonList: List<JSONObject> = emptyList()

                    if (useTools) {
                        // Stage 1: Ask model to choose tools only
                        val toolsPrompt = """
                    System: 
                ...
                """.trimIndent()
                        ...
                        Log.d(TAG, "Tools selection prompt:\n$toolsPrompt")

                        val deferredToolsResult = CompletableDeferred<String>()
                        model.generateResponse(requestId, toolsPrompt, imagePathSelected) {
                            deferredToolsResult.complete(it)
                        }
                        val toolsRawResponse = deferredToolsResult.await()
                    ...
                    // Stage 2: Continue with final response
                    val toolsResultData = if (toolsJsonList.isNotEmpty()) {
                        processTools(toolsJsonList)
                    } else {
                        emptyList()
                    }
            ...
            """.trimIndent()
                    ...
                    Log.d(TAG, "Final prompt:\n$finalPrompt")


                    model.generateResponseAsync(requestId, finalPrompt , imagePathSelected) { resultChunk ->
                    ...
                    Log.e(TAG, "generateResponseAsync error: ${e.message}", e)
                    sendEvent("logging", mapOf(
                        "handle" to handle,
